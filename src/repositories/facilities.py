...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def set_room_facility(self, room_id, facilities_ids: list[int]):
        ...
        delete_facility_ids = list(set(current_facilities_ids) - set(facilities_ids))
        insert_facility_ids = list(set(facilities_ids) - set(current_facilities_ids))
        ...
        if insert_facility_ids:
            query = (
                insert(self.model)
                .values([{'room_id': room_id, 'facility_id': f_id} for f_id in insert_facility_ids])
            )
